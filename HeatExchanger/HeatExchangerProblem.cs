namespace MGroup.FEM.Thermal.Tests.HeatExchanger
{
	...
	public static class HeatExchangerProblem
	{
		[Fact]
		public static void DoublePipeHeatExchanger()
		{
			#region Data
			// DΑΤΑ
			...
			double heatTransferCoeffTube = 40000;
			double heatTransferCoeffShell = 40000;
			double length = 1;
			double diameterInnerTube = 0.05;
			double diameterOuterTube = 0.06;
			double diameterInnerShell = 0.1;
			double thermalConductivityWall = 109;

			//Initial and boundary conditions
			double thetaShellInitial = 300; // shell temperature T(x,0)
			double thetaShellBoundaryL = 300; // shell temperature T(L,t)
			double thetaTubeInitial = 360; // tube temperature T(x,0)
			double thetaTubeBoundary0 = 360; // tube temperature T(0,t)
			double thetaWallInitial = 330; // wall temperature T(x,0)
			double thetaWallBoundary0 = 340; // wall temperature T(0,t)
			double thetaWallBoundaryL = 315; // wall temperature T(L,t)
			#endregion
